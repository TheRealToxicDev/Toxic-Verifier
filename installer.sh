...
function failed {
    ...
    echo -e "\nFailed to install. Please visit https://github.com/TheRealToxicDev/Toxic-Verifier/issues.\n"
    exit 1
}

echo ""
echo "Toxic Verifier Installer - Starting.."
...
directory=$(pwd)
tempinstalldir=Toxic-Verifier-Temp
...
git clone --recursive --depth 1 https://github.com/TheRealToxicDev/ToxicVerifier.git || failed "Cannot install. Ensure you have permissions!"
echo ""
echo "Toxic Verifer has been downloaded!"
...
cd "$directory"
mv "$tempinstalldir"/Toxic-Verifier Toxic-Verifier
rm -r "$tempinstalldir"
